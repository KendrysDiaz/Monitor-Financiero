...
export default function PredictionDesempleo({onClose}) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const values = data.map(dato => dato.Tasa);
    ...
    const fetchData = async (Mes) => {
        try {
            const response = await axios.post('https://tamworth-swift-parrot-msbt.2.us-1.fl0.io/api/v1/desempleo/prediccion', {
                date: {
                    year: selectedYear,
                    month: Mes,
                    day: '30',
            ...
            setValuesP(data.map(dato => dato.Tasa))
        } catch (error) {
    ...
    return <div className='modal_background' >
        ...
        <button className="close_button" onClick={() => onClose(false)}>
            ...
            <div className='modal_graph' style={{ width: '79vw' }}>
                <h2 style={{display:'block'}}>Predicción de Desempleo</h2>
            </div>
